...
const Modal = ({ isOpen, onClose, data, type }) => {
  ...
  // --- FIX 1: Normalize Data (Read 'gallery' OR 'images') ---
  ...
  // --- FIX 3: Image Source Logic (Handle External URLs vs Local Assets) ---
  const getImgSrc = (img) => {
    if (!img) return '';
        ...
        >
          ...
          <div className="flex-1 overflow-hidden p-6">
            <div className="flex flex-col md:flex-row gap-6 h-full">
              ...
              {/* FIX 4: Dynamic Width. If no images, this becomes w-full (Full Width) */}
              <div className={`${(hasImages || type === 'project') ? 'md:w-1/3' : 'w-full'} flex flex-col h-full overflow-hidden transition-all duration-300`}>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-6 scrollbar-thin scrollbar-thumb-gray-800">
                  ...
                  {type === 'achievement' && (
                    <div className="animate-in fade-in slide-in-from-left-4">
                       ...
                       {/* Evidence Counter Badge - Only show if images exist */}
                       {hasImages && (
                        <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 bg-yellow-900/10 border border-yellow-700/30 rounded text-xs text-yellow-500/80 font-mono">
                            <ImageIcon size={14} /> 
                            <span>Gallery: {currentImageIndex + 1} of {images.length}</span>
                        </div>
                       )}
              ...
              {/* FIX 5: Conditional Rendering. Only show if we have images or if it's a project view */}
              {(hasImages || type === 'project') && (
                <div className="md:w-2/3 bg-black/40 rounded-lg border border-gray-800 flex flex-col relative overflow-hidden h-full animate-in fade-in duration-500">
                  
                  {/* ACHIEVEMENT VIEW: CAROUSEL */}
                  {type === 'achievement' && hasImages && (
                     <div className="absolute inset-0 flex items-center justify-center bg-black group">
                        
                        <img 
                          src={getImgSrc(images[currentImageIndex])} 
                  ...
                  {type === 'project' && activeTab === 'overview' && (
                    <div className="p-8 space-y-6 animate-in fade-in h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800">
